    ...
    RageUI.IsVisible(RMenu:Get('policeMenu', 'main'), true, false, true, function()
        ...
        RageUI.Button('Spike Strips', "~b~Place spikes trips", {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
            if (Selected) then
                ...
                TriggerServerEvent("Cyber:placeSpike", netid, coords)
            end
...
RegisterNetEvent("Cyber:addSpike")
AddEventHandler("Cyber:addSpike", function(coords, netid) 
    spikes[netid] = {coords, netid}
end)

RegisterNetEvent("Cyber:deleteSpike")
AddEventHandler("Cyber:deleteSpike", function(netid) 
    spikes[netid] = nil
end)
                ...
                for t = 1, #tires do 
                    ...
                    if #(tirePos-spike[1]) <= 3.6 then 
                        if not IsVehicleTyreBurst(veh, tires[t].index, true) or IsVehicleTyreBurst(veh, tires[t].index, false) then
                            ...
                            TriggerServerEvent("Cyber:removeSpike", spike[2])
                        end
                    end
                end
            end
