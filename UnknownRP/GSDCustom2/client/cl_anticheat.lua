local function BanPlayer(Reason)
    TriggerServerEvent('vRP:BanPlayer', Reason)
end

local in_Spawn_State = false
...
Citizen.CreateThread(function()
    while (true) do
        ...
        if GetEntityHealth(PlayerPedId()) > 200 then
            if not in_Spawn_State then
                BanPlayer("God Mode")
            end
        end

        if GetVehiclePedIsIn(PlayerPedId(), false) ~= 0 then
            if GetPlayerInvincible(GetVehiclePedIsIn(PlayerPedId(), false)) then
                BanPlayer("Vehicle Godmode")
            end
        end
    end
end)
...
Citizen.CreateThread(function()
    ...
    while true do
        if timesDetected >= 100 then
            BanPlayer("Self Healing")
			Citizen.Wait(5000)
        end
